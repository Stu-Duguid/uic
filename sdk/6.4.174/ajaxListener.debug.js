...
TLT.addModule("ajaxListener", function (context) {
    ...
    function maskData(msg) {
        var msgData = msg.customEvent.data,
        ...
        function masker(key, value) {
            var i = fieldBlocklist.length;
            while (i > 0) {
                i -= 1;
                if (fieldBlocklist[i] === key) {
                    return "XXXXX";
                }
                if (fieldBlocklist[i].cRegex) {
                    if (fieldBlocklist[i].cRegex.test(key)) {
                        return "XXXXX";
                    }
                }
            }
        ...
        function maskValues(data) {
            var dataStr;
            try {
                dataStr = JSON.stringify(data, masker);
            } catch (e) {
        ...
        if (msgData.length !== 0) {
            if (blockNonJSONResponse) {
                cType = msgData.responseHeaders["content-type"];
                if (typeof cType === "undefined" || cType.indexOf("application/json") === -1) {
    ...
    function processXHR(xhr) {
        const url = xhr.tListener.url;
        const method = xhr.tListener.method;
    ...
    function isSystemXHR(url) {
        ...
        for (let que of queues) {
            if (que.endpoint && url.indexOf(que.endpoint) !== -1) {
                return true;
            }
        }
        return false;
    ...
    function processConfig(config) {
        let i,
            ...
            filter,
            filters = [];
        const skipSafetyCheck = utils.getValue(config, "skipSafetyCheck", false);
    ...
    return {
        ...
        extractFetchRequestBody: extractFetchRequestBody,
